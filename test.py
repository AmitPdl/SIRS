...
path_sir_wild = './testsets/SIR2/WildSceneDataset/'
path_sir_postcard = './testsets/SIR2/PostcardDataset/' 
path_sir_solid = './testsets/SIR2/SolidObjectDataset/'
path_nature = './testsets/Nature'
...
def creat_list(path,if_gt=True):
    ...
    if if_gt:
        ...
        for _,_,fnames in sorted(os.walk(path)):
            for fname in fnames:
                print(os.path.join(blended_path,fname))
                print(os.path.join(trans_path,fname))
                
                if os.path.join(trans_path,fname).endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff')):
                    image_list.append(os.path.join(blended_path,fname))
                    gt_list.append(os.path.join(trans_path,fname))
                else:
                    continue
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform == True:
            blended, trans = paired_data_transforms(blended,trans)
...
def test_diffdataset(test_loader,save_path=None,if_GT=True):
    ...
    for j, (image, gt) in enumerate(test_loader):
        ...
        with torch.no_grad():   
            ...
            output_t, *_ = gt_model(pretrained_T, pretrained_R)
...
def test_state(state_T,state_R):
    ...
    datasets = odict([('real20', True), ('sir_wild', True), ('sir_solid', True), ('sir_postcard', True)])
